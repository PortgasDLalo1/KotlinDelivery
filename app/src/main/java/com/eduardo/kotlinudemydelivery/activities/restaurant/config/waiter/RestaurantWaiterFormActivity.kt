...
import com.eduardo.kotlinudemydelivery.databinding.ActivityRestaurantWaiterFormBinding
...
class RestaurantWaiterFormActivity : AppCompatActivity() {

    private lateinit var binding: ActivityRestaurantWaiterFormBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityRestaurantWaiterFormBinding.inflate(layoutInflater)
        ...
        toolbar?.title = "Agregar Mesero"
        setSupportActionBar(toolbar)
        supportActionBar?.setDisplayHomeAsUpEnabled(true)

        binding.btnRegisterW.setOnClickListener { registerDelivery() }
    }

    private fun registerDelivery(){
        val name = binding.edittextNombreW.text.toString()
        val lastname = binding.edittextApellidoW.text.toString()
        val email = binding.edittextEmailW.text.toString()
        val phone = binding.edittextPhoneW.text.toString()
        val pass = binding.edittextPasswordrW.text.toString()
        val cpass = binding.edittextPasswordrcW.text.toString()
...
//            Log.d(TAG,"$imageFile")
            usersProvider?.registerWaiter(user)?.enqueue(object : Callback<ResponseHttp> {
                override fun onResponse(
                    call: Call<ResponseHttp>, response: Response<ResponseHttp>
                ) {
                    if (response.body()?.isSuccess == true) {
                        //goToClientHome()
                        Toast.makeText(this@RestaurantWaiterFormActivity, response.body()?.message, Toast.LENGTH_LONG).show()
                        finish()
                    }
                ...
                override fun onFailure(call: Call<ResponseHttp>, t: Throwable) {
                    ...
                    Toast.makeText(this@RestaurantWaiterFormActivity, "Se produjo un error ${t.message}", Toast.LENGTH_LONG).show()
                }
            })
        }
    }
